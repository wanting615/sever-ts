...
let UserController = class UserController {
    static async login(ctx) {
        ...
        const result = { message: '', status: false, data: null };
        if (!body.token) {
            result.message = '参数不能为空';
            ctx.body = result;
            return;
        }
        let paramsStr = rsa_1.default.decrypt(body.token);
        if (!paramsStr) {
            result.message = '参数错误';
            ctx.body = result;
            return;
        }
        let { username, password } = rsa_1.default.strParse(paramsStr);
        if (!username) {
    ...
    static async getUserInfo(ctx) {
        try {
            ...
            const userInfo = await user_1.UserInfoModel.findUserInfo(ctx.query.user_id);
    ...
    static async addUserAddress(ctx) {
        ...
        ;
        if (!phone) {
            result.message = "收货人手机号不能为空";
            return;
        }
        ;
        if (!lng || !lat) {
            result.message = "用户定位失败";
            return;
        }
        ;
        if (!city || !address || !addressName || !addressDetail) {
            result.message = "收货人地址不能为空";
            return;
        }
        ;
        try {
